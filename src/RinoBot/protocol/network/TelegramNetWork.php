...
class TelegramNetWork extends NetWork
{
    ...
    /**
     * 发送信息 (纪念🐱封装的第一个网络协议)
     * @param string $token BotToken
     * @param string $ChatID 聊天ID
     * @param string $ReplyToMessageId (可选)回复消息ID，不是回复消息留空
    ...
    bool $AllowSendingWithoutReply=true) {
        if (empty($ReplyToMessageId)){
            Curl::get($this->api.$Token."/sendMessage?
            chat_id=".$ChatID."&
            text=".$Text."&
     ...
     * @param bool $DisableNotification (可选)静默发送消息，用户将收到无声通知
     */
    public function forwardMessage (string $Token,
    string $ChatID,
    string $FromChatId,
    string $MessageId,
    bool $DisableNotification=false) {
        Curl::get($this->api.$Token."/forwardMessage?
        chat_id=".$ChatID."&
